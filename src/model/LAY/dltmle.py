...
class DeepLTMLE(L.LightningModule):
    ...
    def _get_attention_mask(self, tau, device):
        ...
        mask = torch.full((tau * 2 + 1, tau * 2 + 1), True)
        mask[:, 0] = False
    ...
    def forward(self, W, L, A, Y, a):
        ...
        x = torch.cat([z_W, z_L, z_A], axis=1) # shape: (batch_size, 3*tau, dim_emb+dim_emb_time+dim_emb_type)
        x = self.transformer(x, mask=mask)

        # V[t-1] > A[t-1] > (L[t] > Y[t] > C[t] > V[t] > A[t]) > L(t+1)

        # input:  W[0:1], L[0:tau], A[0:tau]
        # output: _[0:1], G[0:tau], Q[0:tau]
        z_G, z_Q = x[:,1:,:].reshape(batch_size, 2, tau, -1).transpose(0, 1)
        ...
        _, z_Q_a = x[:,1:,:].reshape(batch_size, 2, tau, -1).transpose(0, 1)
